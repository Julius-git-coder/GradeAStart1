...
export const createAnnouncement = async (announcementData) => {
  try {
    ...
    // Add timestamp immediately to ensure proper ordering
    ...
    const docRef = await addDoc(collection(db, "announcements"), {
      ...
      createdAt: serverTimestamp(),
      updatedAt: serverTimestamp(),
      // Add a client timestamp as backup for immediate ordering
...
export const getAnnouncements = async (teamId) => {
  try {
    ...
    announcements.sort((a, b) => {
      const dateA = a.createdAt?.toDate?.() || new Date(a.clientCreatedAt || 0);
      const dateB = b.createdAt?.toDate?.() || new Date(b.clientCreatedAt || 0);
      return dateB - dateA;
    });
...
export const subscribeToAnnouncements = (teamId, callback, onError) => {
  ...
  // Simplified query - just filter by teamId, sort on client
  const q = query(
    collection(db, "announcements"),
    where("teamId", "==", teamId)
  );
  ...
  return onSnapshot(
    ...
    (snapshot) => {
      ...
      announcements.sort((a, b) => {
        const dateA =
          a.createdAt?.toDate?.() || new Date(a.clientCreatedAt || 0);
        const dateB =
          b.createdAt?.toDate?.() || new Date(b.clientCreatedAt || 0);
        return dateB - dateA;
      });
...
export const subscribeToAssignments = (teamId, callback, onError) => {
  ...
  // Simplified query - filter by teamId and status, sort on client
  const q = query(
    collection(db, "assignments"),
    where("teamId", "==", teamId),
    where("status", "==", "active")
